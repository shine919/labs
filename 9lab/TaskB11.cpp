...
void delete_col(int** dest,int rows, int cols, int col){
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            if(j == col){
                dest[i][j] = 0;
...
void check_rows_cols(int** src, int** dest, const int* mas,int& size_mas,int rows, int cols){
        for (int i = 0; i < cols; ++i) {
        int count = 0;
        for (int j = 0; j < rows; ++j) {
            for (int s = 0; s < size_mas; ++s) {
                if (src[j][i] == mas[s]) {
                    count += 1;
//                    s=size_mas;
                }
...
int * ProcessMatrix(int** src, int** dest, int* mas,int& size_mas, int rows, int cols) {
    ...
    // Обрабатываем массив
    check_rows_cols(src, dest,mas,size_mas, rows, cols);
}
